...
export const GeoRiskNavigator: React.FC<GeoRiskNavigatorProps> = ({
    ...
    const [showChart, setShowChart] = useState(true); // Default open for maximum info density
    const [showChanges, setShowChanges] = useState(true); // New audit section
        ...
        <Card className="overflow-hidden border-none shadow-lg">
            ...
            <CardContent className="p-6 bg-white">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <div className="flex items-baseline gap-2">
                            <span className={`text-4xl font-bold ${getScoreColor()}`}>{data.score}</span>
                            <span className="text-sm text-slate-500">/ 100</span>
                        </div>
                    </div>
                    <div className={`px-4 py-2 rounded-full border ${getStatusColor()} flex items-center gap-2`}>
                        {data.level === 'LOW' ? <ShieldCheck className="h-4 w-4" /> : <ShieldAlert className="h-4 w-4" />}
                        <span className="font-bold text-sm">{data.level} Risk</span>
                    </div>
                </div>
                                ...
                                <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
                                    ...
                                    {Array.isArray(data.factors.zone.details) && data.factors.zone.details.length > 0 ? (
                                        <ul className="space-y-1">
                                            {data.factors.zone.details.map((detail, i) => (
                ...
                <p className="mt-4 text-[10px] text-slate-400 italic">
                    *Analysis powered by GeoRisk Navigator™. Last updated: {new Date(data.lastUpdated || Date.now()).toLocaleString()}.
                </p>
            </CardContent>
        </Card >
    );
